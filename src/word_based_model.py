...
class WordBasedClassifier(NLUModel):
    ...
    def get_train_data(self, corpus, labels, shuffle=True):
        ...
        # Shuffle endings 
        if shuffle: 
            shuffled_corpus, shuffled_labels = data.shuffle_endings(corpus, labels) 

        encoded_context, encoded_endings = data.encode_text(shuffled_corpus, self.sentence_len, self.vocab)
    ...
    def get_test_data(self, corpus): 
        ...
        """
        encoded_context, encoded_endings = data.encode_text(corpus, self.sentence_len, self.vocab)
...
if __name__ == '__main__': 
    ...
    neg_endings = np.load('data/incorrect_endings/lm_incorrect_endings.npy')
    ...
    # Model 
    word_base_classifier = WordBasedClassifier(vocab, sentence_len=max_len)

    # Get encoded training data 
    encoded_train_stories, train_labels = word_base_classifier.get_train_data(train_stories, train_labels, shuffle=True)

    # Get encoded validation data 
    encoded_valid_stories = word_base_classifier.get_test_data(valid_stories)
    encoded_test_stories  = word_base_classifier.get_test_data(test_stories)

    # Train the model 
    word_base_classifier.fit(encoded_train_stories, train_labels, encoded_valid_stories, valid_labels, epochs=8, learning_rate=5e-3)

    print('Validation Accuracy: ', word_base_classifier.score(encoded_valid_stories, valid_labels))

    print('Test Accuracy: ', word_base_classifier.score(encoded_test_stories, test_labels))
