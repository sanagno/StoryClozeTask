...
class RNNBaseline(object): 
    ...
    def define_graph(self):
        ...
        with tf.variable_scope("feedforward"):
            # Hidden layer 
            self.logits = self.dense(inputs=outputs[-1], units=1, activation=None, name="hidden_layer")

            # Output layer 
            # self.logits = self.dense(inputs=h, units=1, activation=None, name="output_layer")

            self.probs = tf.math.sigmoid(self.logits)

        with tf.name_scope("loss"):
            self.loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.y, logits=self.logits, name="loss"))

        return self

    def dense(self, inputs, units, activation=None, use_bias=True, 
            kernel_initializer=None, kernel_regularizer=None, batch_norm=False, name=None):
        """
        Customised dense layer to apply batch normalization and dropout.
        """

        y = tf.layers.dense(inputs=inputs,
                            units=units, 
                            activation=None, 
                            use_bias=use_bias, 
    ...
    def optimizer(self, optimizer='rms_prop', learning_rate=0.001, clip_norm=10.0):
        ...
        with tf.name_scope("learning_rate"):
            ...
            self.learning_rate = tf.train.exponential_decay(self.learning_rate, self.global_step, decay_steps, decay_rate, staircase=True)
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(epochs):
                ...
                for i in range(train_steps):
                    ...
                    train_dict = {self.x: batch_x, self.y: batch_y, self.training: True}

                    if i % log_freq == 0:
                        # Training step 
                        _, g_step, loss, summary = sess.run([self.train_step, self.global_step, self.loss, merged_summaries], feed_dict=train_dict)
                        train_loss.append(loss)
                        # _, summary = sess.run([self.train_step, merged_summaries], feed_dict=train_dict)
                        ...
                        batch_x = x_val[j*batch_size:(j+1)*batch_size, :, :]
                        batch_y = y_val[j*batch_size:(j+1)*batch_size]

                        # Get batch prediction 
                        y_val_pred, y_val_pred_proba = self.batch_predictions(sess, batch_x)

                        # Accuracy 
                        valid_acc.append(accuracy_score(batch_y, y_val_pred))
                ...
                _, loss = sess.run([self.train_step, self.loss], feed_dict=train_dict)
                train_loss.append(loss)
    ...
    def batch_predictions(self, session, batch_x):
        ...
        y_pred_proba_e1 = session.run(self.probs, feed_dict={self.x: batch_x_1, self.training: False})
        ...
        y_pred_proba_e2 = session.run(self.probs, feed_dict={self.x: batch_x_2, self.training: False})

        y_pred_proba = np.hstack((y_pred_proba_e1, y_pred_proba_e2))
