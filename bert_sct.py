...
def main(argv):
	...
	shutil.rmtree(flags.save_results_dir, ignore_errors=True)
	os.makedirs(flags.save_results_dir)
	...
	for i in range(flags.num_estimators):
		...
		predictions = get_final_predictions(test['story'].values, test['ending'].values, tokenizer, estimator, label_list)
		val_score = accuracy_score(true_labels_test, combine_predictions(predictions))
